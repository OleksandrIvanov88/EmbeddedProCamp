 ...
 * But handbrake is 0 (released/off)
 * When SHM_AreMovingParametersValid(void) is called
 * Then it  shall return VALID
 */

void test_MovingParameters_MaximumlValues_except_handbrake(void)
{
	BOOLEAN_E actual = INVALID;
...
/**
 * Given all parameters equal to a minimal valid values
 * But handbrake is equal to max 1 (on)
 * When SHM_AreMovingParametersValid(void) is called
 * Then it  shall return VALID
 */

void test_MovingParameters_Handbrak_OutOfMax(void)
{
	BOOLEAN_E actual = INVALID;
...
/**
 * Given speed > 0,, rpm can be any, handbrake is equal 1(on)
 * But gear != 0 (driving)
 * When SHM_AreMovingParametersValid(void) is called
 * Then it  shall return INVALID
 */

void test_MovingParameters_DrivingInGear_with_HandbrakeOn(void)
{
	BOOLEAN_E actual = INVALID;
	actual = SHM_AreMovingParametersValid(1,-1, 0, 1);
	TEST_ASSERT_EQUAL_INT(INVALID, actual);
}


/**
 * Given speed > 0, rpm can be any, handbrake is equal to 1 (on)
 * But gear == 0(neutral)
 * When SHM_AreMovingParametersValid(void) is called
 * Then it  shall return VALID
 */
void test_MovingParameters_DrivingInNeutral_with_HandbrakeOn(void)
{
	BOOLEAN_E actual = INVALID;
	actual = SHM_AreMovingParametersValid(1,0, 0, 1);
	TEST_ASSERT_EQUAL_INT(VALID, actual);
}
